...
Deployer.prototype.deploy = function () {
	...
	this.server.open(function (err) {
        async.series([
            ...
            that._setPermissions.bind(that),
...
// Set the permissions WordPress likes, 755 for dirs and 644 for files
Deployer.prototype._setPermissions = function (cb) {
    ...
    this.server
        ...
        .execQueue(function (err) {
            if (err) cb(err);
                ...
                that.logger.log('Permissions set');
                cb();
            }
        });
